...
import CupSVG from 'assets/3DElements/cup.svg';
...
class SelectFormItemNotFast extends Component {
  ...
  componentDidMount = () => {
    ...
    services.listDictionary('SKL').then((res) => {
  ...
  AddItem = (item) => {
    ...
    let filterSelected = this.state.options.filter((e) => e.id != item.id);

    this.setState((prevState) => ({
      ItemSelected: [...prevState.ItemSelected, item],
      options: [item, ...filterSelected],
    }));
  };

  Fetchfn = async (value) => {
    return services.listAutocompleteDictionary(value, 'SKL');
  };
  ...
  render() {
    ...
    var { options, loading, ItemSelected } = this.state;
    return (
      <View
        style={{
          width: '100%',
            ...
            >
              <View
                style={{
                  ...
                  alignItems: 'center',
                  top: -platform.getResponsiveHeight(6),
                }}
              >
                <CupSVG
                ...
                {!loading && (
                  ...
                  >
                    <FlatList
                      ...
                      numColumns={Math.ceil(options.length / 5)}
