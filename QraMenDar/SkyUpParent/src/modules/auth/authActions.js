...
import Permissions from 'sercurity/roles';
...
const actions = {
  ...
  doSigninSocial: (provider, rememberMe) => async dispatch => {
    try {
      ...
      Linking.addEventListener('url', async (event, params) => {
        ...
        await AsyncStorage.setItem('refresh_token', credentials.refresh_token);
  ...
  doSigninWithEmailAndPassword: (
    ...
    try {
      ...
      if (!permissionChecker.match(Permissions.values.parent)) {
        console.log('Error');

        throw new Error("Don't have access permision");
      }
      ...
      await service.AfterLogin();
      Service.getNotificationCount().then(e => {
        if (e != 0) {
          Notifications.setBadgeCountAsync(e);
        }
      });

      throw 'Request Dazet';

      NavigationService.Reset('MainStack', 'AssignementsNotification');
  ...
  doSignout: () => async (dispatch, getState) => {
    try {
      ...
      await service.signoutConnexion();
      ...
      NavigationService.Reset('Auth', 'Signup');
    } catch (error) {
      console.log(error);
      // Errors.handle(error);
  ...
  doSigninFromAuthChange: () => async dispatch => {
    ...
    try {
      if (refreshToken != null) {
        ...
        NavigationService.Reset('Auth', 'SigninPage');
      }
    } catch (error) {
      console.log(error);
  ...
  doUpdatePassword: (email, currentPassword, newpassword) => async dispatch => {
    try {
      dispatch({
        type: actions.UPDATE_PROFILE_START,
      });
